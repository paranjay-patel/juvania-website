...
- [ ] **Update Config URL:** Update the `site: 'https://juvania.app'` URL in `astro.config.mjs` to match your newly purchased domain for correct canonical URLs and SEO indexing.
- [ ] **Update Hardcoded Links:** Search the codebase for `juvania.app` (e.g., in `Layout.astro` fallback URLs) and update them to the final domain.
